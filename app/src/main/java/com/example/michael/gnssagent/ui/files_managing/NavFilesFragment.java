...
public class NavFilesFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        View rootView = inflater.inflate(R.layout.fragment_nav_files, container, false);

        lv = rootView.findViewById(R.id.navFilesListView);
    ...
    private ArrayList<LogFile> getAllFiles() {
        ...
        if (logFiles.exists() && logFiles.listFiles() != null) {
            for (File file : logFiles.listFiles()) {
                ...
                if (fileNameSplit[fileNameSplit.length-1].equals("nav.txt")) {
                    files.add(f);
                }
            }
        }
